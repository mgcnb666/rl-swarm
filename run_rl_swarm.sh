...
# 添加Testnet连接选项
echo -en $GREEN
read -p ">> 是否要连接到Testnet? [Y/n] " yn
echo -en $NC
yn=${yn:-Y}  # 默认为"Y"
case $yn in
    [Yy]*)  CONNECT_TO_TESTNET=True ;;
    [Nn]*)  CONNECT_TO_TESTNET=False ;;
    *)  echo ">>> 请回答yes或no." && CONNECT_TO_TESTNET=True ;;
esac

if [ "$CONNECT_TO_TESTNET" = "True" ] || [ -f "modal-login/temp-data/userData.json" ]; then
    cd modal-login
    source ~/.bashrc
...
cleanup() {
    ...
    # 确保所有训练进程都被终止
    pkill -f "hivemind_exp.gsm8k.train_single_gpu" 2>/dev/null || true
    exit 0
}
